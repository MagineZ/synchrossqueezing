function [tfr, tfrtic, tfrsq, tfrsq2nd, tfrsqtic] = sqSTFTbase2nd(x, lowFreq, highFreq, alpha, tDS, h, Dh, DDh, online);
%
...
if online 
    tfr = zeros(N/2, 100);     % for h
    tfrsq = zeros(fLen, 100);
    tfrsq2nd = zeros(fLen, 100);
else 
...
for tidx = 1:tLen,
	...
	if online
		tfr(:, 1:99) = tfr(:, 2:100) ;
		tfrsq(:, 1:99) = tfrsq(:, 2:100) ;
		tfrsq2nd(:, 1:99) = tfrsq2nd(:, 2:100) ;
        tfr(:, 100) = tf0(1:N/2) ;
        tfrsq(:, 100) = sst ;
        tfrsq2nd(:, 100) = sst2nd ;
		
		if ~mod(tidx, 10) 
			subplot(1,2,1) ; imageSQ([1:100]', tfrsqtic, abs(tfrsq)) ; 
			subplot(1,2,2) ; imageSQ([1:100]', tfrsqtic, abs(tfrsq2nd)) ; pause(1e-9)
		end
